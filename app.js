...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const menuToggle = document.getElementById('menuToggle'); // Get menu toggle button
    const sidebar = document.querySelector('.sidebar'); // Get sidebar
    const content = document.querySelector('.content'); // Get content area
    ...
    if (menuToggle && sidebar && content) {
        menuToggle.addEventListener('click', function() {
            ...
            // Optionally, add a class to the body or content to adjust styles
            // document.body.classList.toggle('sidebar-active'); 
        });
    }

    // Check if user is logged in - enhanced with token validation
    function checkLoginStatus() {
        ...
        if (token && currentUser) {
            try {
                // In a real app, verify token validity here with the server
                // For demo, just check if token exists and hasn't expired
                ...
                // Valid token, update user profile display
                ...
                if(appContainer) appContainer.style.display = 'flex'; // Ensure it uses flex display
    ...
    // Parse JWT token (simplified without signature validation)
    ...
    function generateToken(user) {
        ...
        // For real implementation, these would be properly signed
        const headerEncoded = btoa(JSON.stringify(header));
        const payloadEncoded = btoa(JSON.stringify(payload));
        
        // In a real app, this would include a proper signature
        const signature = btoa("thisIsAFakeSignatureForDemo");
    ...
    function loadUserData() {
        ...
        // In a real app, this would fetch user-specific data from the server
        // For demo purposes, we're just using localStorage with a user prefix
        
        const userId = currentUser.id;
        const userPrefix = `user_${userId}_`;
        
        // Load user's animals
        const animals = JSON.parse(localStorage.getItem(userPrefix + 'animals') || '[]');
        localStorage.setItem('animals', JSON.stringify(animals));
        
        // Load user's fincas
        const fincas = JSON.parse(localStorage.getItem(userPrefix + 'fincas') || '[]');
        ...
        [
            ...
            const data = JSON.parse(localStorage.getItem(userPrefix + key) || '[]');
    ...
    function saveUserData() {
        ...
        localStorage.setItem(userPrefix + 'animals', JSON.stringify(animals));
        
        // Save user's fincas
        const fincas = JSON.parse(localStorage.getItem('fincas') || '[]');
        localStorage.setItem(userPrefix + 'fincas', JSON.stringify(fincas));
    ...
    // Handle login - enhanced with token generation
    if (loginButton) {
    ...
    // Handle register - enhanced with token generation
    if (registerButton) {
    ...
    // Handle logout with enhanced data saving
    ...
    // Set up auto-save for user data (every 2 minutes)
    setInterval(saveUserData, 2 * 60 * 1000);
    
    // Handle saving data before page unload
    ...
    function createAnimalCard(animalData) {
        ...
        card.setAttribute('data-type', animalData.type || 'regular');
    ...
    function createBirthCard(birthData) {
        ...
        card.setAttribute('data-type', 'birth');
    ...
    function loadFincas() {
        ...
        fincas.forEach(finca => {
            const card = createFincaCard(finca);
    ...
    // Variable para modo de edredo de finca
    ...
    // Manejar subida de archivos para Saneamiento
    const uploadSanitaryBtn = document.getElementById('uploadSanitaryBtn');
    const sanitaryFileInput = document.getElementById('sanitaryFileInput');
    const sanitaryFilesList = document.getElementById('sanitaryFilesList');
    
    if (uploadSanitaryBtn && sanitaryFileInput && sanitaryFilesList) {
        uploadSanitaryBtn.addEventListener('click', function() {
            sanitaryFileInput.click();
        });
        
        sanitaryFileInput.addEventListener('change', function() {
            if (this.files.length > 0) {
                handleFileUpload(this.files, sanitaryFilesList, 'Sanitario');
            }
        });
    }
    ...
    function loadSavedFiles() {
        ...
        // Load Sanitary files
        const sanitaryFilesList = document.getElementById('sanitaryFilesList');
        if (sanitaryFilesList) {
            const sanitaryFiles = JSON.parse(localStorage.getItem('sanitaryFiles') || '[]');
            if (sanitaryFiles.length > 0) {
                sanitaryFilesList.innerHTML = '';
                sanitaryFiles.forEach(file => {
                    const fileItem = createFileItem(file);
                    sanitaryFilesList.appendChild(fileItem);
                    const deleteBtn = fileItem.querySelector('.delete-file');
                    if (deleteBtn) {
                        deleteBtn.addEventListener('click', function() {
                            const fileId = this.getAttribute('data-file-id');
                            const updatedFiles = sanitaryFiles.filter(f => f.id !== fileId);
                            localStorage.setItem('sanitaryFiles', JSON.stringify(updatedFiles));
                            fileItem.remove();
                            if (updatedFiles.length === 0) {
                                const emptyMessage = document.createElement('p');
                                emptyMessage.className = 'empty-list-message';
                                emptyMessage.textContent = 'No hay documentos de saneamiento registrados. Usa el botón "Subir Documento" para añadir documentación.';
                                sanitaryFilesList.appendChild(emptyMessage);
                            }
                        });
                    }
                });
    ...
    function handleFileUpload(files, container, type) {
        const storageKey = type === 'Guía' ? 'guiaFiles' : 
                          type === 'PAC' ? 'pacFiles' : 'sanitaryFiles';
        ...
        Array.from(files).forEach(file => {
            ...
            const fileItem = createFileItem(fileObject);
            ...
            if (deleteBtn) {
                deleteBtn.addEventListener('click', function() {
                    ...
                    if (updatedFiles.length === 0 && container.children.length === 0) {
                        const emptyMessage = document.createElement('p');
                        emptyMessage.className = 'empty-list-message';
                        emptyMessage.textContent = type === 'Guía' ? 'No hay guías registradas' : 
                                                 type === 'PAC' ? 'No hay archivos subidos' : 
                                                 'No hay documentos de saneamiento registrados';
    ...
    function loadTransactions() {
        const birthTableBody = document.getElementById('transactionsTableBody');
        if (!birthTableBody) return;
        
        birthTableBody.innerHTML = '';
        ...
        if (recentTransactions.length === 0) {
            ...
            birthTableBody.appendChild(emptyRow);
            return;
        }
        ...
        recentTransactions.forEach(transaction => {
            ...
            birthTableBody.appendChild(row);
        });
            ...
            setTimeout(() => {
                resultContainer.innerHTML = `
                    ...
                    <div style="margin-top: 20px;">
                        ...
                        <p>Este es un resumen generado mediante los parámetros seleccionados. En una implementación real, estos datos se obtendrían de la base de datos.</p>
...
function openActivityModal(activity = null) {
    ...
    const today = new Date();
    document.getElementById('activityDueDate').min = today.toISOString().split('T')[0];
...
// Cambiar estado de la actividad (completada)
...
function clearAllReportsRecords() {
    if (confirm("¿Está seguro de que desea borrar todos los registros de informes?")) {
        ...
        localStorage.removeItem('sanitaryFiles');
...
function exportAllAnimals() {
    ...
    const exportBtn = document.getElementById('exportAllAnimalsBtn');
    const exportFileName = `animal-records-${new Date().toISOString().split('T')[0]}.json`;
...
function updateAllRecordsTab() {
    ...
    if (allRecordsTab && indexofClass(allRecordsTab.classList, 'active')) {
        loadAllAnimalRecords();
    }
}
...
// New function to load farms from localStorage and render
function createFincaCard(fincaData) {
    const card = document.createElement('div');
    card.className = 'finca-card';
    card.id = fincaData.id;
    ...
    card.innerHTML = `
        <div class="finca-header">
            ...
            <div class="finca-title">
                <h3>${fincaData.name}</h3>
                <span class="finca-type-badge">${fincaData.cattleType || 'Mixto'}</span>
            </div>
        </div>
        ...
        <div class="finca-stats">
            <div class="stat-item">
                ...
                <span class="stat-value">${fincaData.size} ha</span>
            </div>
            <div class="stat-item">
                <span class="stat-label">Animales</span>
                <span class="stat-value" id="animalCount-${fincaData.id}">0</span>
            </div>
        </div>
        ...
        <div class="finca-details">
            <div class="detail-item">
                ...
                <span>${fincaData.location}</span>
            </div>
            <div class="detail-item">
                <i class="fas fa-calendar"></i>
                <span>Creada: ${new Date(fincaData.timestamp).toLocaleDateString()}</span>
            </div>
        </div>
    ...
    // Update animal count for this farm
    updateFarmAnimalCount(fincaData.id);
    
    return card;
}

// Add this helper function to update animal counts
function updateFarmAnimalCount(farmId) {
    const animals = JSON.parse(localStorage.getItem('animals') || '[]');
    const farmAnimals = animals.filter(animal => animal.finca === farmId);
    const countElement = document.getElementById(`animalCount-${farmId}`);
    if (countElement) {
        countElement.textContent = farmAnimals.length;
    }
}